...
- **Modals/chrome:** `ContextMenu` — a Fusion-style **marking menu**: 8 radial quick actions around the cursor (selection-aware; picks close it) + a linear list with PERSISTENT axis-lock toggles (unlike the momentary X/Y/Z keys — Esc still clears), work-plane segment, Translate/Member-dimension/Optimize/Help (segments don't close the menu). Ring center clamps to the viewport. Also `MemberDimensionModal`, `BoxFrameDialog` (state persists across open/close), `HelpModal` (⚠ maintained separately from the real key handler — they can drift), `PanelResizer` (writes raw px; store clamps).
