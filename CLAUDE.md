...
| `profiles.ts` | `ProfileDef`, `ProfileStock`, `makeProfile`, `defaultProfileStock`, `profileForRole`, `PROFILE_PALETTE`, `profileColorAt`. | Section clamped 1–500 mm. `PROFILE_PALETTE` (6 colors) is deliberately separate from templates' `ROLE_PALETTE` — don't unify. |
