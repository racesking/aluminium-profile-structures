...
- `getStructurePayload`/`hydrateFromPayload` enumerate persisted fields **by hand** — adding a field means editing both. (The legacy `saveProject`/`loadProject` localStorage pair was removed; real persistence is `store/autosave.ts` → IndexedDB.)
